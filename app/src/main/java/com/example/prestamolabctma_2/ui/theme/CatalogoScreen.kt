...
fun CatalogoScreen(
    ...
    val categorias = remember(equipos) {
        listOf("Todas") + equipos.map { it.categoria.toString() }.distinct()
    }

    val equiposFiltrados = equipos.filter { equipo ->
        val coincideNombre = equipo.nombre.contains(textoBusqueda, ignoreCase = true)
        val coincideCategoria = categoriaSeleccionada == "Todas" || equipo.categoria.toString() == categoriaSeleccionada
